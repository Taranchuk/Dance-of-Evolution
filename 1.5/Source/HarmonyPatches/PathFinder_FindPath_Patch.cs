...
namespace DanceOfEvolution
{
    ...
    public static class PathFinder_FindPath_Patch
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (!found)
            {
                Log.Error("[DanceOfEvolution] PathFinder.FindPath Transpiler failed. The code won't work.");
            }
        }
        ...
        static public float ChangePathCostIfNeeded(Pawn pawn, int xCell, int zCell, float cost)
        {
            ...
            if (pawn != null && pawn.IsFungalNexus() && pawn.CanPassOver(cell))
            {
        ...
        public static bool CanPassOver(this Pawn pawn, IntVec3 c)
        {
            ...
            {
                Thing thing = list[i];
                if (thing is Plant plant && plant.def.plant.cavePlant)
                {
                    return true;
                }
            }
