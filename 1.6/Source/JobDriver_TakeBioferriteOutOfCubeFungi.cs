...
{
    public class JobDriver_TakeBioferriteOutOfCubeFungi : JobDriver
    {
        ...
        protected Building_CubeFungi CubeFungi => (Building_CubeFungi)job.GetTarget(TargetIndex.A).Thing;

        protected Thing Bioferrite => job.GetTarget(TargetIndex.B).Thing;

        public override bool TryMakePreToilReservations(bool errorOnFailed)
        {
            return pawn.Reserve(CubeFungi, job, 1, -1, null, errorOnFailed);
        }
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            yield return Toils_General.Wait(200).FailOnDestroyedNullOrForbidden(TargetIndex.A).FailOnCannotTouch(TargetIndex.A, PathEndMode.Touch)
                .FailOn(() => CubeFungi.containedBioferrite < 1f)
